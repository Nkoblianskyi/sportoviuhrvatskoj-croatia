...
export function CasinoCard({ casino, rank }: CasinoCardProps) {
  ...
  const renderBadges = () => {
    if (!casino.badges || casino.badges.length === 0) return null

    return (
      <div className="flex flex-wrap gap-2 mb-4">
        {casino.badges.map((badge, idx) => (
          <div
            key={idx}
            className="bg-gradient-to-r from-[#ff6b35] to-[#f7931e] px-3 py-1 text-xs font-bold rounded-full text-white uppercase tracking-wide shadow-lg"
          >
            {badge}
          </div>
        ))}
      ...
      {casino.badges && casino.badges.length > 0 && (
        <div className="absolute -top-5 left-1/2 -translate-x-1/2 z-10 flex gap-2">
          {casino.badges.map((badge, idx) => (
            <div
              key={idx}
              className="bg-gradient-to-r from-[#ff6b35] to-[#f7931e] px-6 py-2.5 text-xs font-black rounded-full text-white uppercase tracking-widest shadow-2xl shadow-[#ff6b35]/50 border-2 border-white/20"
            >
              {badge}
            </div>
          ))}
      ...
      >
        ...
        <div className="hidden md:grid md:grid-cols-[100px_auto_auto_auto] md:gap-8 md:items-center pt-2">
          <div className="text-center flex-shrink-0">
            <div className={`text-6xl font-black ${isTop3 ? "text-[#ff6b35]" : "text-gray-600"}`}>{rank}</div>
          </div>

          <div className="bg-black p-6 rounded-lg border border-gray-800 flex-shrink-0 w-[200px]">
            <Image
          ...
          <div className="flex flex-col items-center justify-center flex-shrink-0 px-6">
            <div className="text-xs uppercase tracking-wider text-[#ff6b35] mb-3 font-bold">Bonus</div>
            <div className="text-3xl font-bold text-white text-center leading-tight">{casino.bonus}</div>
            {casino.dopBonus && (
              <div className="text-sm text-[#f7931e] text-center mt-2 font-semibold">{casino.dopBonus}</div>
            )}
          </div>
          ...
          <div className="flex items-center gap-8 ml-auto">
            <div className="text-center flex-shrink-0">
              ...
              <div className="text-xs text-gray-500 mt-3 font-medium">{casino.reviews} recenzija</div>
            </div>

            <div className="flex-shrink-0">
              <div className="bg-gradient-to-r from-[#ff6b35] to-[#f7931e] hover:from-[#f7931e] hover:to-[#ff6b35] text-white font-bold whitespace-nowrap text-base px-8 py-4 rounded-lg transition-all duration-300 hover:scale-105 shadow-lg shadow-[#ff6b35]/30">
        ...
        <div className="md:hidden space-y-5">
          <div className="flex items-center justify-between gap-4">
            <div className="bg-black p-4 rounded-lg border border-gray-800 flex-shrink-0 w-[160px]">
              <Image
                src={casino.logo || "/placeholder.svg"}
                alt={casino.name}
                width={140}
            ...
            <div className="flex flex-col items-center justify-center flex-1">
              ...
              {casino.dopBonus && (
                <div className="text-xs text-[#f7931e] text-center mt-1 font-semibold">{casino.dopBonus}</div>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              <div className={`text-3xl font-black ${isTop3 ? "text-[#ff6b35]" : "text-gray-600"}`}>{rank}</div>
            ...
            <div className="bg-gradient-to-r from-[#ff6b35] to-[#f7931e] hover:from-[#f7931e] hover:to-[#ff6b35] text-white font-bold text-sm px-6 py-3 rounded-lg flex-shrink-0 transition-all duration-300 shadow-lg shadow-[#ff6b35]/30">
              <span>Igraj</span>
            </div>
          </div>
        </div>
